...
// Based on https://www.habrador.com/tutorials/rope/2-simplified-rope/
...
public class RopeControllerSimple : MonoBehaviour {
    ...
    [HeaderAttribute("Rope Data")]
    [Range(0, 20f)]
    public float ropeLength = 1f;
    [Range(0, 25f)]
    public float minRopeLength = 1f, maxRopeLength = 20f;
    [Range(0, 1f)]
    public float ropeWidth = 0.2f;

    [HeaderAttribute("Spring Data")]
    [Range(5000, 10000f)]
    public float density = 7750f; // Density of the wire (stainless steel) kg/m3
    [Range(0, 0.1f)]
    public float radius = 0.02f; // The radius of the wire
    ...
    void Start() {
        loadMass = whatIsHangingFromTheRope.gameObject.GetComponent<Rigidbody>().mass;
